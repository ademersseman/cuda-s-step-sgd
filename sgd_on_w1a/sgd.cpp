    ...
    int print_interval)
{
    ...
    for (int iter = 0; iter < maxiters; iter++) {
        ...
        float* batch_X = d_X + b * batch_size * n_features;
        float* batch_y = d_y + b * batch_size;

        cudaMemset(d_grad, 0, n_features*sizeof(float));

        compute_gradient(batch_X, batch_y, d_w, d_grad, batch_size, n_features);
        update_weights(d_w, d_grad, lr, n_features, batch_size);
...
int main() {
    ...
    train(dX, dy, dw, n_samples, n_features, batch_size, maxiters, lr, hX, hy, print_interval);
