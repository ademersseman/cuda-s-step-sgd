...
__global__ void scale_X_by_y_kernel(float *A, float *y, float *X_scaled, int total_samples, int n_features)
{
    ...
    if (i < total_samples)
    {
        ...
        {
            X_scaled[i * n_features + j] = A[i * n_features + j] * yi;
        }
    }
}
...
extern "C" void compute_sstep_gradient(float *A, float *y, float *x, float *grad, int batch_size, int s, float eta, int n_features)
{
    ...
    scale_X_by_y_kernel<<<blocks_samples, BLOCK_SIZE>>>(A, y, d_X_scaled, total_samples, n_features);
    cudaDeviceSynchronize();
    ...
    // Apply sigmoid to first block (i=0) BEFORE using it in corrections
